...
void LecteurVue::viderDiaporama()
{
    ...
    cout << /*nbImages()*/0 << " images restantes dans le diaporama." << endl;
...
void LecteurVue::affichageInfosApp()
{
    ...
    msgBox->setText("Version de l’application : Version 7_BD_VitesseDefilement\nDate de création : 05/06/2023\nAuteurs : Maxime MONTOURO et Clément MOURGUE");
...
void LecteurVue::demandeModifVitesse()
{
    ...
    {
        this->m_fVitesse = new formVitesseDefilement(this,this->m_donnees[this->m_indiceDiapo]->getVitesseDefilementBase());
        this->m_fVitesse->exec();
        if (this->m_fVitesse->getAccepte())
        {
            this->m_vitesseDefilement = this->m_fVitesse->getVitesseDefilement();
            if (!this->m_mydb->isAlreadyOpen())
            {
                this->m_mydb->openDataBase();
            }
            QSqlQuery req("UPDATE Diaporamas SET vitesseDefilement = :vitDef WHERE idDiaporama = :idDiapo ;");
            req.bindValue(":vitDef",this->m_vitesseDefilement);
            req.bindValue(":idDiapo",this->numDiaporamaCourant());
            if (!req.exec())
            {
                QMessageBox::critical(this,"Erreur !","Erreur lors de la mise à jour de la vitesse de défilement dans la base de données !");
            }
            //this->m_mydb->closeDataBase();
        }
     }
}
